    ...
    extends StochasticAgent
{
    ...
    public void valueIteration(StateView state) {
        ...
        Map<Coordinate, Double> utilitiesP = new HashMap<>(utilities);
    
        double maxChange;
        do {
            maxChange = 0.0;

            for (Map.Entry<Coordinate,Double> entry: utilitiesP.entrySet()) {
                utilities.put(entry.getKey(), entry.getValue());
            }
    
            for (Coordinate c : utilities.keySet()) {
                if (isTerminalState(c)) {
                    utilitiesP.put(c, RewardFunction.getReward(c));
                    continue;
                } 
                    ...
                    for (Direction d : TransitionModel.CARDINAL_DIRECTIONS) {
                        ...
                        for (Pair<Coordinate, Double> transition : TransitionModel.getTransitionProbs(state, c, d)) {
                            //double reward = RewardFunction.getReward(transition.getFirst());
                            double nextUtility = utilities.get(transition.getFirst());
                            totalUtility += transition.getSecond() * (nextUtility);
                        }
                        if (totalUtility > maxUtility) {
                            maxUtility = totalUtility;
                        }
                    }
                    double newUtility = RewardFunction.getReward(c) + GAMMA * maxUtility;
                    if (Math.abs(newUtility - utilities.get(c)) > maxChange) {
                        maxChange = Math.abs(newUtility - utilities.get(c));
                    }
                    utilitiesP.put(c, newUtility);
            }
        ...
        System.out.println("utilities: " + this.getUtilities());
    }
