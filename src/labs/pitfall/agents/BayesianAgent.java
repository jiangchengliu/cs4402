        ...
        extends Object
    {
         ...
         **/
         ...
         public Coordinate getNextCoordinateToExplore() {
            if (this.getFrontierPitCoordinates().isEmpty()) {
                return null;
            }
            List<Coordinate> minProbCoordinates = new ArrayList<>();
            double minProb = Double.MAX_VALUE;
            double totalProb = 0f;
            for (Coordinate coord : this.getFrontierPitCoordinates()) {
                double baseProb = this.pitProb;
                double factor = 0;
                for (Coordinate adj : getNeighbors(coord)) {
                    Boolean breeze = this.knownBreezeCoordinates.get(adj);
                    if (breeze != null) {
                        factor += breeze ? 1 : -1;
                    }
                }
                totalProb += factor;
                double prob = (baseProb + factor * baseProb)/totalProb;

                if (prob < minProb) {
                    minProb = prob;
                    minProbCoordinates.clear();
                    minProbCoordinates.add(coord);
                } else if (prob == minProb) {
                    minProbCoordinates.add(coord); 
                }
            }
            Random rand = new Random();
            return minProbCoordinates.get(rand.nextInt(minProbCoordinates.size()));
        }
        ...
        private List<Coordinate> getNeighbors(Coordinate coord) {
            ...
            for (Direction dir: Direction.values()) {
                int x = coord.getXCoordinate() + dir.xComponent();
                int y = coord.getYCoordinate() + dir.yComponent();
